...
static long fast_strtol_dec(char** str) {
   long result = 0;
   bool neg = false;
...
static unsigned long fast_strtoul_dec(char** str) {
...
static long long fast_strtoll_dec(char** str) {
   long long result = 0;
   bool neg = false;
...
static inline uint64_t fast_strtoull_dec(char** str) {
...
static bool LinuxProcessTable_readStatFile(LinuxProcess* lp, openat_arg_t procFd, const LinuxMachine* lhost, bool scanMainThread, char* command, size_t commLen) {
   ...
   /* (4) ppid  -  %d */
   Process_setParent(process, fast_strtol_dec(&location));
   location += 1;

   /* (5) pgrp  -  %d */
   process->pgrp = fast_strtol_dec(&location);
   location += 1;

   /* (6) session  -  %d */
   process->session = fast_strtol_dec(&location);
   location += 1;

   /* (7) tty_nr  -  %d */
   process->tty_nr = fast_strtoul_dec(&location);
   location += 1;

   /* (8) tpgid  -  %d */
   process->tpgid = fast_strtol_dec(&location);
   location += 1;

   /* (9) flags  -  %u */
   lp->flags = fast_strtoul_dec(&location);
   location += 1;

   /* (10) minflt  -  %lu */
   process->minflt = fast_strtoull_dec(&location);
   location += 1;

   /* (11) cminflt  -  %lu */
   lp->cminflt = fast_strtoull_dec(&location);
   location += 1;

   /* (12) majflt  -  %lu */
   process->majflt = fast_strtoull_dec(&location);
   location += 1;

   /* (13) cmajflt  -  %lu */
   lp->cmajflt = fast_strtoull_dec(&location);
   location += 1;

   /* (14) utime  -  %lu */
   lp->utime = LinuxProcessTable_adjustTime(lhost, fast_strtoull_dec(&location));
   location += 1;

   /* (15) stime  -  %lu */
   lp->stime = LinuxProcessTable_adjustTime(lhost, fast_strtoull_dec(&location));
   location += 1;

   /* (16) cutime  -  %ld */
   lp->cutime = LinuxProcessTable_adjustTime(lhost, fast_strtoull_dec(&location));
   location += 1;

   /* (17) cstime  -  %ld */
   lp->cstime = LinuxProcessTable_adjustTime(lhost, fast_strtoull_dec(&location));
   location += 1;

   /* (18) priority  -  %ld */
   process->priority = fast_strtol_dec(&location);
   location += 1;

   /* (19) nice  -  %ld */
   process->nice = fast_strtol_dec(&location);
   location += 1;

   /* (20) num_threads  -  %ld */
   process->nlwp = fast_strtol_dec(&location);
   location += 1;
   ...
   if (process->starttime_ctime == 0) {
      process->starttime_ctime = lhost->boottime + LinuxProcessTable_adjustTime(lhost, fast_strtoll_dec(&location)) / 100;
   } else {
   ...
   /* (39) processor  -  %d */
   process->processor = fast_strtol_dec(&location);
...
static void LinuxProcessTable_readMaps(LinuxProcess* process, openat_arg_t procFd, const LinuxMachine* host, bool calcSize, bool checkDeletedLib) {
   ...
   while (fgets(buffer, sizeof(buffer), mapsfile)) {
      ...
      map_inode = fast_strtoull_dec(&readptr);
      if (!map_inode)
         continue;
...
static void LinuxProcessTable_readOomData(LinuxProcess* process, openat_arg_t procFd, const LinuxProcess* mainTask) {
   ...
   char* oomPtr = buffer;
   uint64_t oom = fast_strtoull_dec(&oomPtr);
   if (*oomPtr && *oomPtr != '\n' && *oomPtr != ' ') {
      return;
   }
