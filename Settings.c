...
static bool Settings_read(Settings* this, const char* fileName, unsigned int initialCpuCount) {
   ...
   for (;;) {
      ...
      } else if (String_eq(option[0], "shadow_trusted_path_prefix")) {
         this->shadowTrustedPathPrefix = atoi(option[1]);
...
int Settings_write(const Settings* this, bool onCrash) {
   ...
   fprintf(fd, "shadow_trusted_path_prefix=%d\n", (int) this->shadowTrustedPathPrefix);
...
Settings* Settings_new(unsigned int initialCpuCount) {
   ...
   this->shadowTrustedPathPrefix = false;
